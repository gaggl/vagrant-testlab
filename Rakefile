...
task :inspec, [:node] do |t, args|
  ...
  nodes.each do |node|
    ...
    if(File.directory?("test/#{name}"))
      ...
      sh("bundle exec cinc-auditor exec test/#{name} --sudo -t ssh://vagrant@127.0.0.1:$(vagrant port --guest 22 #{name}) -i #{vagrant_root}/.vagrant/machines/#{name}/virtualbox/private_key")
    else
